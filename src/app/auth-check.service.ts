...
export class AuthCheckService {
  ...
  //my edit
  public wasFirstLogIn(): boolean {
    let access_token = JSON.parse(localStorage.getItem('access_token'));
    let status = (Object.keys(access_token).length === 1) ? true : false;
    console.log('hello: ' + Object.keys(access_token).length)
    return status;
  }
  ...
  public isUserProfileComplete() {
    ...
    if (!status) {
      ...
      //my edit
    } else if (this.wasFirstLogIn()) {
      document.getElementById("trialMessageModalShowButton").click();
    } else {
      return
    }
  }
